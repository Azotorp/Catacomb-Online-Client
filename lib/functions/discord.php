...
function discord_auth_url($clientid, $redirect, $scope)
{
    $state = gen_state();
    return 'https://discord.com/api/oauth2/authorize?client_id='.$clientid.'&redirect_uri='.$redirect.'&response_type=code&scope='.$scope.'&state='.$state;
    //return 'https://discordapp.com/oauth2/authorize?response_type=code&client_id='.$clientid.'&redirect_uri=&scope=' . htmlspecialchars($scope) . "&state=";
}

# A function to initialize and store access token in SESSION to be used for other requests
function discord_init($redirect_url, $client_id, $client_secret, $bot_token=null)
{
    if ($bot_token != null)
...
function get_user($token = "")
{
    ...
    $ip = get_ip();
    $_SESSION['user_auth_key'] = sha1($_SESSION['username'].SECRETSALT.$_SESSION['discrim'].SECRETSALT.$_SESSION['user_id'].SECRETSALT.$ip);
}
